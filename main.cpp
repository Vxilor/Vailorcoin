...
#if defined(NDEBUG)
# error "Vailorcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1cef4e48222517f849d7a6ca3e32599167c9383e9c55176778be29414e9fb543");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Vailorcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Vailorcoin Signed Message:\n";
...
{
    // Vailorcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Vailorcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Vailorcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Vailorcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Vailorcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Vailorcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe1f9713187fdb68405900d4b67b26259bb6bc39098b35c0ab512114596bafac4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "/9/7/2021 VailorCoin Created!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d0e20bd247e206345568a2156cb20c7a62cc84a1e456970ee1c99e5bff7e400129f0e44731906c0f60b95aa43899f2f1a4840d0f4da8ac4713c2832fb5d062f5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1631007427;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084644669;

        if (fTestNet)
        {
            block.nTime    = 1631007409;
            block.nNonce   = 386245380;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xce993363060ed8dbebe0257084272a08fa097be2e22038c05154d473b6916c3b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb4, 0xdc }; // Vailorcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VailorcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VailorcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VailorcoinMiner(CWallet *pwallet)
{
    printf("VailorcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vailorcoin-miner");
    ...
    try { loop {
        ...
        printf("Running VailorcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VailorcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VailorcoinMiner, pwallet));
}
